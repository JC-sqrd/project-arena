...
func _ready():
	...
	owner.ready.connect(
		func():
			...
			shield_bar.max_value = health_manager.max_health.stat_derived_value + health_manager.current_shield.stat_derived_value
			value = health_manager.current_health.stat_derived_value 
			shield_bar.value = health_manager.current_health.stat_derived_value + health_manager.current_shield.stat_derived_value
			...
			if under_healthbar != null:
				under_healthbar.max_value = max_value#health_manager.max_health.stat_derived_value
				under_healthbar.value = value#health_manager.current_health.stat_derived_value
				pass
	)
	pass
...
func update_health_ui(new_current : float, new_max : float):
	...
	if new_current > value:
		...
		if under_healthbar != null:
			under_healthbar.max_value = new_max + health_manager.current_shield.stat_derived_value  
			...
			tween.finished.connect(
				func():
						...
						if health_manager.current_health.stat_derived_value <= health_manager.max_health.stat_derived_value:
							value = health_manager.current_health.stat_derived_value
							shield_bar.max_value = health_manager.max_health.stat_derived_value + health_manager.current_shield.stat_derived_value
							shield_bar.value = value + health_manager.current_shield.stat_derived_value
