...
func _init(distance : float, bonus_value : float, relative_to_actor : bool, greater_than : bool, is_multiplier : bool):
...
func condition_met(hit_data : Dictionary) -> bool:
	...
	if relative_to_actor:
		relative_to = hit_data.get("actor") as Node2D
	else :
		relative_to = hit_data.get("source") as Node2D
