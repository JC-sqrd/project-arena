...
func spawn_damage_number(damage_taken : float, damage_type : Enums.DamageType):
	#var damage_taken
	#var damage_type
	...
	if damage_type == Enums.DamageType.PHYSICAL:
		text_color = Color.INDIAN_RED
	elif damage_type == Enums.DamageType.MAGIC:
		text_color = Color.CORNFLOWER_BLUE
	else:
		text_color = Color.WHITE
	damage_number.global_position = owner.global_position + (Vector2.UP * 25) #+ (Vector2.RIGHT * randi_range(-10,10))
	damage_number.text = str(floorf(damage_taken))
	damage_number.z_index = 5
	damage_number.scale = Vector2.ONE * 1.5
	damage_number.label_settings = LabelSettings.new()
	damage_number.self_modulate = text_color
	
	damage_number.label_settings.font_color = "#FFF"
	...
	pos_tween.tween_property(damage_number, "position", damage_number.position + (Vector2.UP * 100), 0.6)
